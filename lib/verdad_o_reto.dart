...
class _VerdadRetoRouteState extends State<VerdadRetoRoute> {
  ...
  void loadRetos() async {
    String retos = await rootBundle.loadString("assets/texts/reto.txt");
    _retos = LineSplitter.split(retos).toList();
    _retos.shuffle();
  }

  void loadVerdades() async {
    String verdades = await rootBundle.loadString("assets/texts/verdad.txt");
    _verdades = LineSplitter.split(verdades).toList();
    _verdades.shuffle();
  }
